...
# Strict Guard Rail validation check
if not GEMINI_API_KEY:
    print("[❌ CRITICAL] GEMINI_API_KEY environment variable is missing or empty!")
    sys.exit(1)

if not TELEGRAM_TOKEN:
    print("[❌ CRITICAL] TELEGRAM_TOKEN environment variable is missing or empty!")
    sys.exit(1)

# Explicitly initialize Client using verified key string
...
def load_live_configs():
    """Reads configuration rules fallback safety net."""
    try:
...
def analyze_with_ai(headline, details):
    """Uses Google's standard gemini-2.0-flash production model."""
    prompt = f"""
    ...
    try:
        ...
        print(f"[❌ API Error] Authentication credentials rejected: {ae}")
        return "Routine", "⚪ Neutral", f"- API Auth/Config Error: Check your deployment Secrets token values."
    except Exception as e:
        print(f"[⚠️ AI Exception] Handshake failed: {e}")
        return "Routine", "⚪ Neutral", f"- Summary extraction framework issue: {e}"
...
def check_feed_cycle(is_baseline=False):
    ...
    try:
        ...
        for item in reversed(items):
            ...
            # Clean symbols to prevent markdown text formatting breaks
            ...
            time.sleep(1) # Graceful spacing pacing
...
def trigger_workflow_handover():
    if not GH_PAT or not GITHUB_REPOSITORY:
        print("[Handover] GitHub environments details missing. Stopping loop chain workflow pipeline.")
        return
    ...
    try:
        ...
        if res.status_code == 204:
            ...
            print(f"[❌] Handoff runner deployment initialization failed: {res.status_code} - {res.text}")
    except Exception as e:
        print(f"[❌] Fatal link terminal failure during handover pipeline transition: {e}")
